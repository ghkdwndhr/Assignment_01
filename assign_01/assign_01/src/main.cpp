...
void loop() {
  ...
  // ✅ 실시간 밝기 조절
  brightness = map(analogRead(potPin), 0, 1023, 50, 255);
...
void trafficLightTaskCallback() {
  ...
  unsigned long totalCycleTime = redDuration + yellowDuration + greenDuration + 1000 + yellowDuration;
  ...
  if (t < redDuration) {
    ...
    greenBlinkTask.restart();  // 깜빡이기 시작

  } else if (t < totalCycleTime) {
    analogWrite(redLedPin, 0);
    analogWrite(yellowLedPin, brightness);  // 🔥 깜빡임 후 노란불
    analogWrite(greenLedPin, 0);
    redState = false; yellowState = true; greenState = false;

  } else {
    previousMillis = millis();  // 🔁 다음 주기 시작
  }
}
